...
{
	if (getGrade() <= _form.getGradeToExe()){
		std :: cout << getName() << " executed " << _form.getName() << std::endl; 
		_form.execute(*this);
	}
	else
		std::cout << getName() << " couldn't execute " << _form.getName() <<
		" because " << getGrade() << " is Lower then "<< _form.getGradeToExe() << std::endl;
}
